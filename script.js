  ...
  if (carouselSlides.length > 0) {
    ...
    if (mobileCallBtn) {
      mobileCallBtn.addEventListener('click', (e) => {
        ...
        const confirmCall = confirm(`Would you like to call Crystal Springs B&B at ${telNumber}?`);
        if (!confirmCall) {
    ...
    const scrollSections = [
      ...
      { id: 'breakfast', element: document.getElementById('breakfast') },
      { id: 'amenities', element: document.getElementById('amenities') },
      { id: 'gallery', element: document.getElementById('gallery') },
      { id: 'killarney', element: document.getElementById('killarney') },
    ...
    const updateActiveLink = () => {
      ...
      desktopNavLinks.forEach(link => {
        ...
        if (href === `#${activeSectionId}`) {
          isActive = true;
        } else if (href === '#rooms' && ['rooms', 'offers', 'amenities'].includes(activeSectionId)) {
          isActive = true;
        } else if (href === '#killarney' && ['killarney', 'location'].includes(activeSectionId)) {
          isActive = true;
        }
